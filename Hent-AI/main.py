...
# Initialize Q-table and state visit counts with zeros, and move to GPU if available
Q_table = torch.zeros(num_states, num_actions, dtype=torch.float32).to(device)
state_visit_counts = torch.zeros(num_states).to(device)
...
try:
    ...
    while True:
        ...
        # Calculate reward
        exploration_boost = 3.0 / (1.0 + state_visit_counts[current_state])
        reward = (
            completion_reward if completion_reward_data == 1
            else step_penalty - 0.01 * state_visit_counts[current_state]
        ) + exploration_boost + end_goal_boost
        ...
        if is_goal_reached:
            print("Maze completed! Reached the end goal.")
            break
